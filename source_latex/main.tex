...
In this report the properties of the Clark Y airfoil are empirically explored at varying angles of attack by measuring airfoil surface and wake pressure distributions in a subsonic wind tunnel. The center of pressure distributions, a momentum balance to determine the total drag, and numerical methods to calculate the coefficients of lift, (pressure) drag, and moment ($C_L$, $C_D$, and $C_M$, respectively) are calculated. The measurements for these calculations are collected using two separate methods, and their comparative effectiveness is evaluated.\newline

Though analysis it was found that the Clark Y airfoil appeared to stall at $\alpha\approx 13^\circ$. The $C_P$ plot showed a large plateau region for $\alpha\geq13^\circ$ indicating significant follow separation. For the same region, the $C_L$ curve experienced a significant drop in the lift slope and the $C_D$ curve had a sudden increase, both characteristic of a stalled airfoil. No useful information was extractable from the $C_M$ curve which remained relatively constant with respect to the angle of attack.\newline

The empirically attained airfoil characteristics reflected the general trends and magnitudes of the the values calculated from XFOIL and reported in literature. This suggests that the results, within an experimental error, accurately reflect the true behaviour of a Clark Y airfoil in uniform flow at varying angles of attack.
...
\noindent
Pressure is obtained via pre-processing procedures described in section \ref{sec:procedure}, and further normalized using equation \ref{eq:cp}. The free stream velocity $U_\infty$ is taken to be the mean between the measured velocity at the uppermost (port 20) and bottommost (port 36) rakes. Figure \ref{fig:cp} illustrates the $C_p$ distribution over the chord when $\alpha = 0^\circ, 6^\circ $ and $15^\circ$. Results of all other $C_p$ distribution are attached in Appendix \ref{app:CP}. \newline

\noindent
Overall, the trend of the $C_p$ distribution fits the theoretical prediction. The upper wing has a more negative pressure distribution than the lower wing, which in turn provides lift. The $C_p$ changes drastically at the small section directly succeeding the leading edge. In lower angles of attack, the $C_p$ changes gradually after the leading edge, but in a higher angle of attack, a pressure plateau is visible on graphs due to stalling effects. \newline
...
Table \ref{tab:quant_error} contains sources of quantifiable error and their associated uncertainty.
...
This appendix details the error propagation formulas employed throughout the experiment.

\subsection{Scanivalve Calibration}

The uncertainty of the pressure difference associated with scanivalve calibration can be found in the following:
...
\noindent
Note that the precision error of $\delta V$ for Scanivalve measurements is given by autocorrelation analysis on port 36 for each angle of attack.

\subsection{Manometer Pressure}

The manometer pressure calculations use a reference height of $h_R = 18.2\;\si{cm}$ that we assume to be errorless for a baseline. In these calculations, $\theta = 61 \pm 0.1^\circ$.
...
For cases where the airfoil velocity at a single pitot tube is desired, we can calculate the velocity and its uncertainty as follows:
...
An average free stream velocity and its uncertainty can be obtained by the following methods that use the pressure measurements from ports 20 and 36. Here we assume zero error in $\rho$.
...
The velocity deficit is a tool used to create the wake velocity distribution. Its calculation and uncertainty is given by the following:
...
The total drag force was evaluated numerically using the trapezoidal rule.
...
The following gives the dynamic pressure and its uncertainty assuming no error for the free stream density.

\begin{align*}
    q_\infty &= \frac{1}{2} \rho_\infty U_\infty^2\\
    \delta q_\infty &= \rho_\infty U_\infty \delta U_\infty
\end{align*}
...
The normal force experienced by the airfoil and its uncertainty can be calculated by the following:
...
\noindent
In practice, the uncertainty of the angle $\theta$ is assumed to be zero as its calculation is dependent only on the tap positions with which we assume have no error.

\subsection{Axial Force}

% Assumptions include that our tap locations are known perfectly and thus we have no theta uncertainty.

The axial force experienced by the airfoil and its uncertainty can be calculated by the following:
...
The moment force experienced by the airfoil and its uncertainty can be calculated by the following:
...
The lift uncertainty can be calculated using the following where the uncertainty associated with the angle of attack is $\delta\alpha=\pm 0.25^\circ$.
...
The pressure drag uncertainty can be calculated using the following where the uncertainty associated with the angle of attack is $\delta\alpha=\pm 0.25^\circ$.
...
The coefficient and its uncertainty can be calculated as follows:
...
The coefficient of lift is calculated using the following:
...
\noindent
In practice, we assume that the free stream density, and the chord length are known with no error.

\subsection{Coefficient of Drag}

The coefficient of drag is calculated using the following:
...
The coefficient of moment is calculated using the following:
