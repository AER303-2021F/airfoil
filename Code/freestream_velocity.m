...
d_p_mano = 10;
% Manometer error, Pa
d_p_scani = 9.80665 * 0.0063;
% Maximum error from all scanivalve time series, Pa
...
for i = 1:11
    ...
    d_v_inf_scanivalve(i) = sqrt((d_p_scani/(2*v_inf_scanivalve(i)*RHO))^2 + (d_p_scani/(2*v_inf_scanivalve(i)*RHO))^2);
    
    combined_rakes_manometer = [rake_b_manometer(i,:); rake_a_manometer(i,:)];
    combined_rakes_manometer = combined_rakes_manometer(:);
    v_inf_manometer(i) = 1/2 * (sqrt(2 * combined_rakes_manometer(1) / RHO) + sqrt(2 * combined_rakes_manometer(end) / RHO));
    d_v_inf_manometer(i) = sqrt((d_p_mano/(2*v_inf_manometer(i)*RHO))^2 + (d_p_mano/(2*v_inf_manometer(i)*RHO))^2);
